--Storming Duplicity Mara
if not Rune then Duel.LoadScript("proc_rune.lua") end
local s,id=GetID()
function s.initial_effect(c)
	--Rune Summon
	Rune.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_WIND),1,1,nil,1,1)
	c:EnableReviveLimit()
	--tohand
...
function s.thcfilter(c,tp)
	return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(1-tp) and c:IsPreviousLocation(LOCATION_ONFIELD)
		and (c:GetPreviousTypeOnField()==TYPE_SPELL or c:GetPreviousTypeOnField()==TYPE_TRAP)
end
